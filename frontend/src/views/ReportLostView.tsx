...
export default function ReportLostView() {
  ...
  return (
    <div className="max-w-4xl mx-auto" dir={dir}>
      <div className="mb-8">
        <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-3">{t('lost_page_title')}</h1>
        <p className="text-lg text-gray-600">
          {t('lost_page_subtitle')}
        </p>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-lg p-6 sm:p-8 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">
              {t('lost_form_where_question')} <span className="text-red-500">*</span>
            </label>
            <select
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">
              {t('lost_form_when_question')} <span className="text-red-500">*</span>
            </label>
            <select
              value={formData.when}
      ...
      {showMatches && (() => {
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">{t('lost_ai_matches_title')}</h2>
          
          {/* Match Count Message */}
          {filteredMatches.length > 0 ? (
            <p className="text-gray-600 mb-6">
              {t('lost_matches_found').replace('{count}', filteredMatches.length.toString())}
            </p>
          ) : (
            <p className="text-gray-600 mb-6">
              {t('lost_ai_matches_empty')}
            </p>
          )}
          ...
          {filteredMatches.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 text-lg">
                No matches yet. If someone reports a similar found item, you'll see it here.
              </p>
            </div>
          )}
        </div>
